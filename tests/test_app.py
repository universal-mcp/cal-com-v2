...
@pytest.fixture
def app_instance():
    """Provides a CalComV2App instance for tests."""
...
def test_universal_mcp_cal_com_v2_app_initialization(app_instance):
    """
    Test that the CalComV2App instance is initialized correctly with a name.
    """
    ...
    assert app_instance.name == "cal-com-v2", "CalComV2App instance has unexpected name."


def test_universal_mcp_cal_com_v2_tool_docstrings_format(app_instance):
    """
    Test that each tool method in CalComV2App has a well-formatted docstring,
...
def test_universal_mcp_cal_com_v2_tools_are_callable(app_instance):
    """
    Test that each tool method returned by list_tools in CalComV2App is callable.
    """
    tools = app_instance.list_tools()
    assert isinstance(tools, list), "list_tools() should return a list."
